const
    types = exports,
    { is, constants: { $$instance } } = require('.'),
    util = require('@pfoerdie/utility'),
    _define = (obj, key, value) => Object.defineProperty(obj, key, { value });

types.number = (val) => parseFloat(val);
_define(types.number, $$instance, is.number);
types.string = (val) => '' + val;
_define(types.string, $$instance, is.string);
types.function = (val) => is.function(val) ? val : () => types.number(val);
_define(types.function, $$instance, is.function);
types.array = (val) => is.array(val) ? val
    : val?.[Symbol.iterator] ? Array.from(val)
        : (val ?? null) === null ? [] : [val];
_define(types.array, $$instance, is.array);

types.int = (val) => parseInt(val);
_define(types.int, $$instance, is.number.integer);
...
_define(types.int64, $$instance, is.number.bigint);

types.uint = (val) => Math.abs(types.int(val));
_define(types.uint, $$instance, (val) => is.number.integer(val) && val >= 0);
...
_define(types.float, $$instance, is.number);
